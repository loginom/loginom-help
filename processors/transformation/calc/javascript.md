...
Как и в компоненте JavaScript поддерживаются внешние модули, но в *Калькуляторе* возможно использование только модульных систем [CommonJS](http://wiki.commonjs.org/wiki/Modules/1.1.1). Модульные системы [ES6](https://www.ecma-international.org/ecma-262/6.0/#sec-modules) (ECMAScript 6) не поддерживаются. С примерами и документацией использования модулей CommonJS можно ознакомится в статье [JavaScript: Импорт внешних модулей](../../../processors/transformation/java-script/external-modules.html#modulnye-sistemy).
...
Инициализация объектов модуля происходит при первом таком вызове, при последующих используется кэш модуля, в котором сохраняются состояния его объектов после предыдущего вызова.  Поскольку в *Калькуляторе* вычисление ячеек таблицы происходит в цикле по строкам и столбцам, то вызов модуля как правило происходить многократно с использованием кэша.  
Это необходимо учитывать, и при необходимости очищать кэш, доступный через объект `require.cache`.
...
> **Важно**: использование кэша для передачи значений между вызовами модуля может привести к непредсказуемым результатам вычислений, в виду использования многопоточности вычислений.
