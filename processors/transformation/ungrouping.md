# ![](../../media/app/icons/component-18/component-default-16.svg) Разгруппировка
...
* ![](../../media/app/icons/ports/output-table-inactive.svg) Разгруппируемые данные - таблица, содержащая поле для разгруппировки.

* ![](../../media/app/icons/ports/output-table-inactive.svg) Данные для расчета долей - таблица, из которой считываются значения долей для разгруппировки.

### Выход

* ![](../../media/app/icons/ports/output-table-inactive.svg) Выход разгруппировки - исходная таблица после обработки, в нее добавляются следующие поля:
...
* ![](../../media/app/icons/ports/output-table-inactive.svg) Показатели качества модели и сезонные индексы - справочная таблица, составляется только при использовании метода "с учетом временных колебаний и сезонности опорных данных", содержит поля:
 ...
 * ![](../../media/app/icons/toolbar-18/usagetype-18-06.svg) Группа - такое назначение выставляется автоматически при связывании полей групп. Связывать следует поля групп, одинаковые для обеих таблиц - для этого достаточно перетащить метку поля из одной таблицы на соответствующую метку поля из другой таблицы; если образованная при этом связь ошибочна, удалить ее можно нажатием на расположенную на ней кнопку ![](../../media/app/icons/toolbar-18/toolbar-delete-join.svg);
 * ![](../../media/app/icons/view-types-18/facts.svg) Разгруппируемое - назначение выставляется полю таблицы разгруппируемых данных, к которому и будет применена разгруппировка;
 * ![](../../media/app/icons/toolbar-18/usage-column.svg) Поле с наименованиями - выставляется полю опорной таблицы, содержащему наименования, по которым будет производиться разгруппировка;
 * ![](../../media/app/icons/toolbar-18/usagetype-18-08.svg) Поле с долями - выставляется полю опорной таблицы, содержащему доли или значения для расчета долей;
 * ![](../../media/app/icons/toolbar-18/ordered-field.svg) Порядковое поле - данное назначение доступно только для полей типа "дата-время" при использовании метода "с учетом временных колебаний и сезонности опорных данных".
