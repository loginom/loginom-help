...
Компонент исполняет процесс, по своей сути обратный процессу агрегации по сумме, применяемому в [группировке](/app/processors/transformation/grouping.md). Для определения групп, по которым будет производиться обратная агрегация численного поля, требуется опорная таблица, из которой заимствуются сами группы и учитывается процентное распределение значений численного поля между ними.
 ...
 * ![](/mediaapp/icons/ports/output_table_inactive.svg) Показатели качества модели и сезонные индексы - справочная таблица, составляется только при использовании метода "с учетом временных колебаний и сезонности опорных данных", содержит поля:
