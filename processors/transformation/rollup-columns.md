...
Выполним свертку столбцов. Поля `Код клиента` и `Месяц` выберем как *Информационные*. А `Кол-во SMS (целый тип)`, `Кол-во минут вх (целый тип)`, `Статус (строковый тип)`, `Наличие скидки (логический тип)`, веберем *Траспонируемыми*.
