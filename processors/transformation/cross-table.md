# ![Кросс-таблица](../../images/icons/vendors/crosstab.svg) Кросс-таблица
...
* ![Выходной источник данных](../../images/icons/ports/input_table_inactive.svg) **Выходной набор данных** — на порт выводится таблица с обработанными данными.
...
* **Выбранные поля** — делится на группы.
  * ![Колонки](dataset_action_18) Колонки.
  * ![Строки](dataset_action_18) Строки.
  * ![Факты](dataset_action_18) Факты.
...
* **Разделитель частей меток полей** — выбирается каким символом будут разделены метки новых полей, при выборе нескольких полей в колонках.
  ...
  * ->.
  * Пробел.
* **Скользящие уникальные значения** — установка флага повторяет одноименный параметр, но применяется ко всем колонкам сразу.
* **Ограничение количества значений** — ограничивает максимальное количество колонок.

С помощью кнопок ![Вверх](../../images/icons/toolbar-controls_18x18/toolbar-controls_18x18_moveup_default.svg) и ![Вниз](../../images/icons/toolbar-controls_18x18/toolbar-controls_18x18_movedown_default.svg) можно менять порядок полей в группе. То, в каком порядке расположены поля, влияет на структуру результирующей таблицы.
