...
* По умолчанию выполнение узлов *Python* в серверных версиях запрещено в разделе [Администрирование->Параметры->Параметры компонента Python](../../../admin/parameters.md). В параметре раздела *Путь библиотеки* указывается путь к библиотеке python3x.dll, где x – номер минорной версии. Должна использоваться библиотека с полным API. Если путь не задан, то осуществляется автоматический ее поиск в переменной окружения `PYTHONHOME` и реестре.
