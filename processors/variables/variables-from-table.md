...
* для чисел будет выбрана *сумма*;
* для остальных — *количество*.
...
Имена, метки и значения будут получены следующим образом:

* **Имя переменной** — будет совпадать с именем поля, если была выбрана лишь одна функция. Если функций было больше одной, то к имени добавится окончание, соответствующее выбранной функции.
* **Метка переменной** — будет состоять из имени поля и названии функции агрегации.
* **значение переменной** — формируется из всех значений поля, агрегированных выбранной функцией.
