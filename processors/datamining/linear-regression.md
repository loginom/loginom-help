...
* Выходное поле может быть только одно. Оно должно быть непрерывным и с вещественным типом данных. Выходные данные не должны содержать пропусков во время обучения.
...
* Приоритет точные/недостоверные данные:
   ...
   * Доступно для методов *Ridge*, *Elastic-Net*.
...
* Приоритет меньше/больше факторов:
   ...
   * Может принимать следующие значения:
     ...
     * Больше факторов.
     * Максимум факторов.
   * Доступно для методов *Forward*, *Backward*, *Stepwise*, *LASSO*, *Elastic-Net*.
...
Детальные настройки линейной регрессии включаются при установке соответствующего флажка в настройках линейной регрессии и объединяются в следующие блоки параметров:

#### Настройки метода

Доступные параметры:
* Точность решения — настройка, которая позволяет выбрать, с какой точностью требуется выполнить решение;
