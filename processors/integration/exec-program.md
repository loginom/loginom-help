...
>**Важно:** Этот компонент небезопасный, так как предоставляется доступ к программам и файловой системе! Права на эту операцию задаются в ["Администрировании"](../../admin/parameters.md).
...
* **Тайм-аут выполнения узла** (мсек.) — устанавливает время выполнения узла:
  ...
  * **Бесконечность** — в этом случае узел будет выполняться до тех пор, пока программа не будет закрыта. Для того, чтобы установить время выполнения узла в значение "Бесконечность" необходимо в поле тайм-аута ввести либо -1, либо удалить все цифры.
