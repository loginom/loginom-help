...
Для построения информативного куба нужно перенести поля в область списков измерений
...
> **`(добавить про вложенные измерения)`** - начало.

Количество измерений может варьироваться от нуля до количества доступных полей. Уже используемые измерения можно вернуть обратно в список доступных полей. Рекомендуется использовать не более 5-7 измерений, чтобы отчет был понятным и интерпретируемым.

Так как визуализатор Куб представляет собой плоскую двумерную таблицу, то для отображения нескольких измерений используется древовидная система. Каждое следующее измерение становится элементом предыдущего.

> **`(добавить про вложенные измерения)`** - конец.
