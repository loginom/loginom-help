...
* **Область оценок классификации** — содержит таблицы с классификаторами.

> **Примечание**: *Область настроек* и *Область оценок классификации* можно скрыть, если щелкнуть по середине линии разделителя.
...
* **ROC-кривая** — график показывает соотношение между долей истинно положительных исходов (TPR, Чувствительность) от ложно отрицательных исходов (FPR, 1 - Специфичность).
* **PR-кривая** — график показывает кривую зависимости истинно положительных исходов (TPR, Чувствительность) и количества верно предсказанных положительных примеров (PPV, Точность положительного прогноза).
* **Базовые показатели** — на этой диаграмме строятся графики истинно положительных исходов (TPR, Чувствительность), истинно отрицательных исходов (TNR, Специфичность), ложно положительных исходов (FPR, 1 - Специфичность), ложно отрицательных исходов (FNR, 1 - Чувствительность) и отображается порог отсечения. Диаграмма строится либо для обучающего, либо для тестового множества.
* **Диаграмма точности** — на этой диаграмме строятся графики верно предсказанных положительных примеров (PPV, Точность положительного прогноза), верно предсказанных отрицательных примеров (NPV, Точность отрицательного прогноза), средним значением между PPV и NPV (OPR, Усредненная точность прогноза), (OCR, Общая точность классификации) и отображается порог отсечения. Диаграмма строится либо для обучающего, либо для тестового множества.
* **Диаграмма равновесия** — на этой диаграмме строятся графики верно предсказанных положительных примеров (PPV, Точность положительного прогноза), истинно положительных исходов (TPR, Чувствительность) и отображается порог отсечения. Диаграмма строится либо для обучающего, либо для тестового множества.
...
Чекбокс *Кумулятивный* становится активным при выборе диаграмм: *% распознанных событий*, *Диаграмма роста* и *Диаграмма отклика*. По умолчанию флажок на чекбоксе проставлен, при снятии флажка становится доступным выбор диапазонов в выпадающем списке. Доступные значения: *10 диапазонов*, *20 диапазонов* и *50 диапазонов*. По умолчанию в списке выбрано *10 диапазонов*.
...
> **Примечание**: в зависимости от свободного пространства области, легенда будет располагаться по диаграммой или справа от нее.
...
Таблица разделена на на две части и имеет следующие строки:

* **Оценки классификации** — эта часть содержит оценки которые рассчитываются для всей модели и не зависят от *Порога отсечения*:
  * **AUC ROC** — выводит [площадь под ROC-кривой](https://ru.wikipedia.org/wiki/ROC-кривая#Площадь_под_кривой) для указанного множества.
  * **AUC PR** — выводит площадь под PR-кривой для указанного множества.
  * **Коэффициент Джини** — значение [индекса Джини](https://wiki.loginom.ru/articles/gini-index.html) для указанного множества.
  * **KS** — значение критерия [теста Колмогорова-Смирнова](https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test#Two-sample_Kolmogorov%E2%80%93Smirnov_test) для указанного множества.
...
> **Примечание**: над верхним правым углом таблицы расположен переключатель ![]()/![]()*Абсолютное или отрицательное значение* при переключении которого данные в таблице будут представлены в процентах или в количестве записей попавших в категорию.

### Распознано

В данной таблице показано отношение правильно распознаных событий к общему количеству событий в множествах.
