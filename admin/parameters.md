...
|Период проверки соединения (сек.)|300|Временной промежуток между проверкой соединения с сервером, необходимый для автоматического определения разрыва сетевого соединения и закрытием соответствующей сессии клиента вместе с открытыми в ней пакетами. Проверку соединения с сервером можно отключить, для этого необходимо стереть значение периода проверки и сохранить настройки. Отключение проверки соединения может привести к блокировки пакетов. В случае включенного параметра и небольшого таймаута, связь после разрыва соединения может не восстановиться.|
