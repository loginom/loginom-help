...
Здесь задаются настройки [LDAP](./ldap.md):

|Параметр|Значение по умолчанию|Описание|
|:-|:-|:-|
|Хост||Имя или IP-адрес LDAP сервера. По умолчанию задан пустой строкой и в этом случае не производится проверка LDAP пользователей. |
|Порт|389|Номер порта для подключения к сервису LDAP. По умолчанию 389 для обычного соединения или 636 для SSL соединения.|
|Безопасность соединения|Обычное соединение|Возможность задать защищенное (SSL/TLS) соединение к LDAP по специальному порту (636 по умолчанию) или получение защищенного соединения (STARTTLS) на обычном порту через StartTLS. По умолчанию обычное (незащищенное) соединение.|
|Использовать SASL|false|Используется для защищенной передачи пароля по незащищенному соединению|
...
|Фильтр LDAP|(objectClass=user)|Фильтр пользователей, в котором может быть так же произведена проверка принадлежности пользователя группе. Для Active Directory может быть `(&(objectClass=user)(memberOf:1.2.840.113556.1.4.1941:=cn=users,ou=groups,dc=domain,dc=com))`, для OpenLDAP `(objectClass=posixAccount)`, где использование расширения `:1.2.840.113556.1.4.1941:` дает возможность использовать сопоставление и по вложенным группам (можно использовать только для Active Directory, но не в OpenLDAP). Для фильтров в OpenLDAP может потребоваться дополнительная настройка OpenLDAP: [Group based LDAP login](https://www.easyredmine.com/documentation-of-easy-redmine-2018?view=knowledge_detail&id=89&category_id=8#Group%20based%20LDAP%20login).
|Атрибут с именем пользователя|sAMAccountName|Имя атрибута в записи пользователя, который содержит имя пользователя используемое при подключении и создании "на лету". Для Active Directory это обычно `sAMAccountName`, а для OpenLDAP `uid`. |
