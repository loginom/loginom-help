...
|Период проверки соединения (сек.)|300|Временной промежуток между проверкой соединения с клиентом, необходимый для автоматического определения разрыва сетевого соединения и закрытием соответствующей сессии клиента вместе с открытыми в ней пакетами. Проверка состояния канала связи сервера с клиентом реализуется с помощью служебного сообщения `Ping`, которое периодически отправляется клиенту в том случае, если в течение заданного интервала времени на сервер не приходит ни одного сообщения с клиента. Если же с клиента приходит любое сообщение, то таймер сбрасывается, т.е. сообщение `Ping` отправляется только в том случае, если по каналу связи долго не приходило ни одного входящего сообщения. Проверку соединения с сервером можно отключить, для этого необходимо стереть значение периода проверки, сохранить настройки и перезагрузить сервер. Отключение проверки соединения может привести к блокировки пакетов. В случае включенного параметра и небольшого таймаута, связь после разрыва соединения может не восстановиться. По умолчанию период отправки сообщений составляет 300 секунд. Это означает, что в нормальных условиях сообщение `Ping` никогда не отправляется, т.к. клиент каждые 60 секунд запрашивает у сервера список удалённых объектов, в результате чего 300-секундный таймер сбрасывается.|
