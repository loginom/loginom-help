...
|Атрибут с именем пользователя|sAMAccountName|Имя атрибута в записи пользователя, который содержит имя пользователя используемое при подключении и создании "на лету" (автоматическое создание пользователя Loginom при успешной LDAP аутентификации). Для Active Directory это обычно `sAMAccountName`, а для OpenLDAP `uid`. |
