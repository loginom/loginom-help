...
|Аутентификация|Выбор аутентификации пользователя: Локальная или [LDAP](./parameters.md#ldap). |
...
>
> * Запрещено менять способ авторизации самому себе, так как в этом случае пользователь может стать недоступным, а он мог быть единственным администратором.
>
...
>
> * При включенной LDAP аутентификации у пользователя возможно автоматическое ("на лету") добавление пользователя Loginom в случае успешной аутентификации.
>
> * При смене изменении Аутентификации пользователя с LDAP на Локальная происходит сбрасывание пароля и следовательно пользователь может авторизоваться с пустым паролем, если при смене способа Аутентификации не был задан иной пароль.
