...
* Второе состояние производный узел получает в случае, если какие-либо его свойства были переопределены(изменены) пользователем, в результате чего он станет отличным от базового узла. В этом состоянии унаследованы от базового узла и, соответственно, обновлены при его изменении могут только те свойства, которые пользователь не переопределял.
