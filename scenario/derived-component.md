...
* Первое состояние они имеют сразу после создания. Они абсолютно идентичны базовому узлу и находятся в состоянии активного слежения за его изменениями. При этом все изменения, осуществленные с базовым узлом, произойдут и с самими производными узлами — на них появится оповещение ![](../images/icons/informer/error_warning.svg) "Базовый узел изменился...". Эти изменения вступят в силу:
  * при выборе в контекстном меню производного узла пункта меню "Обновить конфигурацию узла";
  * при активации, переобучении, вызова мастера настройки производного узла или его портов.
* Второе состояние производный узел получает в случае, если какие-либо его свойства были переобределены(изменены) пользователем, в результате чего он станет отличным от базового узла. В этом состоянии унаследованы от базового узла и, соответственно, обновлены при его изменении могут только те свойства, которые пользователь не переопределял.

Чтобы принудительно сбросить все внесенные пользователем изменения и сделать производный и базовый узлы идентичными, следует в контекстном меню производного узла выбрать пункт "Восстановить исходную конфигурацию узла".
